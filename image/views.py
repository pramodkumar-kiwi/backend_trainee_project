...
which represents an individual image.
...
from image.constants import IMAGE_URL_TEMPLATE, MEDIA_URL, IMAGE_PATH_TEMPLATE, IMAGE_GALLERY_PATH
from image.messages import SUCCESS_MESSAGES
from image.models import ImageGallery, Image
from image.serializers import ImageGallerySerializer, ImageGalleryCreateSerializer, \
...
class ImageGalleryViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        It also deletes the associated image folder if it exists.
...
class ImageViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            image_url = IMAGE_URL_TEMPLATE.format(MEDIA_URL, image.image.name)
            return Response(
                {'message': SUCCESS_MESSAGES['IMAGE']['CREATED_SUCCESSFULLY'],
                 'data': {'image': image_url, 'image_gallery_id': image.image_gallery_id}},
                status=status.HTTP_201_CREATED
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
